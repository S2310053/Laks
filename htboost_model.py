...
# We run the model twice: once optimising L2 (RMSE-equivalent), once optimising MAE
##
...
def _fit_htboost(X_train, y_train, X_test, feature_names, ht_loss, purge_weeks=0):
    # Drop columns with >30% NaN in training — HTBoost handles NaN internally,
...
def purged_cv(data, target, features, purge_weeks, n_folds, ht_loss):
    n         = len(data)
    fold_size = n // n_folds
    results   = []
    ...
    for i in range(1, n_folds):
        ...
        try:
            preds, _, _, best_depth, ntrees = _fit_htboost(
                ...
                test[features].values, features, ht_loss, purge_weeks
            )
...
for ht_loss, loss_label in LOSS_FNS:
    ...
    RESULTS_DIR = f"Results/HTBoost/{loss_label}"
    ...
    for target in Y_COLS:
        ...
        print(f"\n[{loss_label}] {label}  (horizon={horizon}, purge={purge_wks}w, folds={n_folds})")

        # Purged CV (HTBoost auto-tunes depth internally via modality="compromise")
        print("Running purged CV:")
        fold_results = purged_cv(cv_data, target, ALL_FEAT, purge_wks, n_folds, ht_loss)
        ...
        hold_preds, final_output, final_data, final_depth, final_ntrees = _fit_htboost(
            ...
            hold_data[ALL_FEAT].values, ALL_FEAT, ht_loss, purge_wks
        )
        ...
        # Feature importance via HTBrelevance (HTBoost's native permutation-based importance)
        try:
        ...
        fig.suptitle(f"HTBoost [{loss_label}]  —  {target}{'  [NOWCAST]' if is_nowcast else ''}",
        ...
        if len(importance) > 0:
            imp_top = importance.head(20).sort_values()
            axes[ax_idx].barh(imp_top.index, imp_top.values, color=_BLUE, alpha=0.85)
            axes[ax_idx].set_title(
                f"Top 20 Feature Importances — HTBrelevance  (depth={final_depth})", fontsize=10)
    ...
    # Summary table
    print(f"\n Summary of HTBoost [{loss_label}] results:")
