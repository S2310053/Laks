# This module constructs the XGBoost model

##
# We construct the XGBoost model per Y horizon, using the same purged k-fold CV setup as CatBoost
# XGBoost (Chen & Guestrin, 2016) is a scalable gradient boosting framework
# We use the same depth grid and early stopping strategy as CatBoost for a fair comparison
##
...
# XGBoost settings (mirroring CatBoost base parameters)
XGB_BASE = dict(
    n_estimators     = 1000,
...
def purged_cv(data, target, features, purge_weeks, n_folds, depth):
    ...
    for i in range(1, n_folds):
        ...
        model = XGBRegressor(**XGB_BASE, max_depth=depth, early_stopping_rounds=EARLY_STOP_ROUNDS)
        model.fit(
            train_inner[features], train_inner[target],
            eval_set=[(val_inner[features], val_inner[target])],
            verbose=False,
        )
...
for target in Y_COLS:
    ...
    final_model = XGBRegressor(**XGB_BASE, max_depth=depth, early_stopping_rounds=EARLY_STOP_ROUNDS)
    final_model.fit(
        cv_train_final[ALL_FEAT], cv_train_final[target],
        eval_set=[(cv_val_final[ALL_FEAT], cv_val_final[target])],
        verbose=False,
    )
...
# Summary table
print("\n Summary of XGBoost model results:")
summary_df = pd.DataFrame(summary).set_index("Y")
print(summary_df.to_string())
summary_df.to_csv(f"{RESULTS_DIR}/xgboost_summary.csv")
...
Plotter().results_table(
    ...
    f"{RESULTS_DIR}/xgboost_results.pdf",
    width=16,
)
